...
const entityIndex = {id: 1, type: 1}
const groupIndex = {group_name: 1, owner: 1}
...
MongoStorage.prototype.init = function (storageConf) {
  ...
  return mongoClient.connect('mongodb://' + storageConf.user + ':' + storageConf.password + '@' + storageConf.host + '/' + storageConf.dbName).then(db => {
      ...
      return new Promise((resolve, reject) => {
        that.entities.createIndex({id: 1, type: 1}, {unique: true}, (err, res) => {
          resolve()
        })
      })
    }).then(() => {
      ...
      that.groups = gColl
    return new Promise((resolve, reject) => {
      return that.groups.createIndex({group_name: 1, owner: 1}, {unique: true}, (err, res) => {
        resolve()
      })
    })
    }).then(() => {
      ...
      throw createError(500, 'Could not connect to mongodb')
    })
};
      ...
      if (action_type === 'entities' || action_type === 'groups') {
        return transaction.indexInformation((err, indexes) => {
          if (err && err.code === 26) {
            ...
            return new Promise((resolve, reject) => {
              transaction.createIndex(action_type === 'entities' ? entityIndex : groupIndex, {unique: true}, (err, res) => {
                resolve()
              })
            })
          } else {
