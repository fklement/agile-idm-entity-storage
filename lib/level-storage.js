...
LevelStorage.prototype.listEntitiesByAttributeValueAndType = function (attribute_constraints, entity_type) {
  ...
  function keyAction(key) {
    return entity_type === key.entity_type;
  }
  ...
  function dataAction(attribute_constraints,value) {
    ...
    attribute_constraints.forEach(function(attribute_constraint){
      passes = passes && (value.hasOwnProperty(attribute_constraint.attribute_type) &&  value[attribute_constraint.attribute_type] === attribute_constraint.attribute_value);
    });
    return passes;
  }
  return iterateDbAndSearch("entities",entity_type?keyTrue:keyAction, dataAction.bind(this,attribute_constraints), transaction(this.entities), true);
