...
var MongoStorage = require('../lib/mongo-storage')
var fs = require("fs");
var onlydb;
const storageConf = {
  "type": "mongodb",
...
function createMongoStorage(finished) {
  return new Promise((resolve, reject) => {
    if (onlydb) {
      ...
      MongoStorage.prototype.cleanDb = function cleanDb(storage, cb) {

        function clean(that, action_type) {
          return new Promise(function (resolve, rej) {
            storage.emptyCollectionPromise(action_type).then(() => {
              resolve()
            })
          }, () => {
            cb()
        ...
        Promise.all([clean(this, storageConf.entityCollection), clean(this, storageConf.groupCollection)]).then(function (data) {
      ...
      onlydb = new MongoStorage();
      onlydb.init(storageConf).then(db => {
        resolve(db)
      })
    }
  })
...
describe('MongoStorage', function () {
  ...
  afterEach(() => {
    createMongoStorage().then(storage => {
      storage.cleanDb(storage, () => {console.log('DB cleaned, next...')})
    })
  })
